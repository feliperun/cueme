...
enum ProjectWorkspaceStore {
    ...
    static func relocate(_ value: MemoryNote, to project: KnowledgeProject?) -> MemoryNote? {
        var record = value
        let source = SessionStore.archiveDirectory(for: record)
        ...
        let destination = SessionStore.rootURL.appendingPathComponent(relative, isDirectory: true)
        do {
            if let project { _ = save(project) }
            ...
            return SessionStore.save(record) == nil ? nil : record
        } catch {
            return nil
        }
    }
