# Golden — the corpus `AGENTS.md`

CueMe writes this at the bundle root **only when it is absent, and never
overwrites it**. It is the operational schema that travels with the corpus, so an
agent pointed at the folder can work it without CueMe. Like the reference OKF wiki
template, it is expected to evolve with the user — overwriting would erase their
edits.

It is not part of the OKF bundle and needs no frontmatter.

> **Naming note.** This golden lives here as `corpus-agents.md`, not `AGENTS.md`,
> so agent harnesses working on *this repository* do not mistake the corpus schema
> for their own directory instructions. The file CueMe writes is named `AGENTS.md`.
